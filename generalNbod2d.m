...
m = [mE 0.7*mE 0.9*mE 0.8*mE, 0.75*mE];
...
svinit = [r1init;v1init;r2init;v2init;r3init;v3init;r4init;v4init;r5init;v4init];
% You can hardcode this. svinit and m are the only things needed

%---Sim Times--------------------------------------------------------------
tEnd = seconds(years(2));           % End time
tScale = seconds(years(1/12));      % Sim timescale. How long in-sim per second irl?
dt = seconds(days(1/2));            % How long between each calculated frame. Mostly just to make sure that lines appear smooth. Must be < tscale/framerate
fps = 30;                           % Framerate of the animation
rotrate = 15;                       % deg/sec pan rate for the animation

useCoM = true;                      % whether to use CoM as origin. Unless you know what you're doing, keep this true
useTex = true;                      % Whether to use LaTeX labels (otherwise matlab Tex used). Currenty, "false" will lead to stupid looking plots

% Note: search for ANIMATIONCOMMENT for stuff to toggle based on whether
% saving the animation or not
...
x=zeros(length(t),numbods);y=zeros(length(t),numbods);z=zeros(length(t),numbods);
vx=zeros(length(t),numbods);vy=zeros(length(t),numbods);vz=zeros(length(t),numbods);
for idx = 1:numbods
    x(:,idx) = SV(:,idx*6-5);
    y(:,idx) = SV(:,idx*6-4);
    z(:,idx) = SV(:,idx*6-3);
